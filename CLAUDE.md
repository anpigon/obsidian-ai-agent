...
This file provides guidance for AI assistants working with the obsidian-ai-agent codebase.

## Project Overview

**obsidian-ai-agent** is an Obsidian plugin that integrates Claude AI directly into the Obsidian workspace. It uses the Claude Agent SDK (`@anthropic-ai/claude-agent-sdk`) to provide an AI chat interface in Obsidian's right sidebar, allowing users to interact with Claude while working in their vault.
...
- **Interfaces**: PascalCase (e.g., `ChatMessage`, `AIChatSettings`)
- **Methods/Functions**: camelCase (e.g., `handleSendMessage`, `renderMessage`)
- **Constants**: UPPER_SNAKE_CASE for true constants (e.g., `VIEW_TYPE_AI_CHAT`, `DEFAULT_SETTINGS`)
- **CSS Classes**: kebab-case with `ai-` prefix (e.g., `ai-chat-container`, `ai-message-content`)
...
### CSS Conventions
- All CSS classes prefixed with `ai-` to avoid conflicts
